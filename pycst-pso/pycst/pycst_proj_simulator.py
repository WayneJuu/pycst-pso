...
from tubepol_ant_prof_pycst_cfg import optimizer_cfg_dic
...
class PyCstProjSimulator:
    ...
    def __init__(self, proj_cfg_dic, data_analyser_cfg_dic, optimizer_cfg_dic):
        ...
        self.optimizer_cfg_dic = optimizer_cfg_dic
        # Project configuration
        # Settings from users
        self.env = proj_cfg_dic.get('env', 'win')
        ...
        self.project_folder = proj_cfg_dic.get('project_folder',
                                               "C:\\Users\\WayneJu\\Desktop\\Project\\02_CST_Project\\TubePolAntProfileRelativeLimScale_Python_Demo")
        ...
        self.optimizer_ins = self.func_opt_pso_create(optimizer_cfg_dic)

        self.pso_eval_id = proj_cfg_dic.get('init_pso_eval_id', 0)
    ...
    def run_pso_optimization(self, max_iter):
        best_cost, best_params = self.optimizer_ins.optimize(self.func_cst_proj_sim_pso, max_iter)
        return best_params, best_cost

    def func_opt_pso_create(self, optimizer_cfg_dic):
        def generate_initial_swarm(initial_position, swarmsize, scale=0.1):
            swarm = np.zeros((swarmsize, len(initial_position)))
            swarm[0, :] = initial_position
            for i in range(1, swarmsize):
                for j in range(len(initial_position)):
                    new_position = initial_position[j] + scale * np.random.uniform(-1, 1)
                    while new_position < 0 or new_position > 1:
                        new_position = initial_position[j] + scale * np.random.uniform(-1, 1)
                    swarm[i, j] = new_position
            return swarm

        # 获取PSO算法的参数init_pos=initial_swarm
        initial_position = np.array(optimizer_cfg_dic['cma_init_para_list'])
        lower_bounds = np.zeros(len(optimizer_cfg_dic['cma_init_para_list']))
        upper_bounds = np.ones(len(optimizer_cfg_dic['cma_init_para_list']))
        maxiter = optimizer_cfg_dic.get('pso_maxiter', 100)
        swarmsize = optimizer_cfg_dic.get('pso_swarmsize', 50)
        initial_swarm = generate_initial_swarm(initial_position, swarmsize)

        # 调用PSO优化
        options = {'c1': 0.5, 'c2': 0.3, 'w': 0.9}
        bounds = (lower_bounds, upper_bounds)
        optimizer = ps.single.GlobalBestPSO(n_particles=swarmsize, dimensions=len(initial_position),
                                            options=options, bounds=bounds, init_pos=initial_swarm)
        #  best_cost, best_params = optimizer.optimize(objective_function, maxiter)
    ...
    def func_cst_sim_run(env, solver, gpu_num, full_para_file, full_cst_file):
        ...
        if env == "linux_pc":  # Linux_PC:
            ...
            cmd_cst = "cst_studio cst_design_environment -numthreads=${NSLOTS} " \
            ...
            #           '-m -par \"%s\" %s \"%s\"' % (full_para_file, solver_cmd, full_cst_file)
            # cmd_cst = '"C:\\Program Files (x86)\\CST Studio Suite 2021\\CST DESIGN ENVIRONMENT.exe" ' \
            #          '-m -par \"%s\" %s \"%s\"' % (full_para_file, solver_cmd, full_cst_file)
            cmd_cst = '"D:\\CST STUDIO SUITE 2022\\CST DESIGN ENVIRONMENT.exe" ' \
                      '-m -par \"%s\" %s \"%s\"' % (full_para_file, solver_cmd, full_cst_file)
        elif env == "test":
            cmd_cst = ""
        else:
    ...
    def func_cst_proj_sim(self, opt_para_val_vec):
        ...
        # swarmsize = optimizer_cfg_dic.get('swarmsize', 50)
        # for i in range(swarmsize):
        # cst_para_val_vec = opt_para_val_vec[i, :]
        ...
        self.func_cst_opt_res_save(project_folder, self.rec_para_file_name, self.rec_objval_file_name,
                                   cst_para_val_vec, self.rec_para_name_lst,
    ...
    def func_cst_proj_sim_pso(self, opt_para_val_vec):
        para_vec = opt_para_val_vec[self.pso_eval_id, :]
        fitness = self.func_cst_proj_sim(para_vec)
        self.pso_eval_id += 1
        swarmsize = optimizer_cfg_dic.get('swarmsize', 50)
        self.pso_eval_id %= swarmsize
        return fitness
